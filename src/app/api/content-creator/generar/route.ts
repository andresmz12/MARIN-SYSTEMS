...

{
  "mapaJson": {
    "centro": {
      ...
      "color": "#1e3a5f",
      "explicacion": "2-3 oraciones que presentan el tema de forma clara y sencilla. ¿Qué es? ¿A quién ayuda?"
    },
    "ramas": [
      {
        ...
        "hijos": [
          { "id": "r1h1", "texto": "punto clave (4-6 palabras)", "color": "#fca5a5", "explicacion": "1-2 oraciones específicas sobre este punto." },
          { "id": "r1h2", "texto": "punto clave (4-6 palabras)", "color": "#fca5a5", "explicacion": "1-2 oraciones específicas sobre este punto." }
        ]
      },
      {
  ...
  "guion": "El guion completo aquí. Debe tener marcadores: [INTRO] [R1] [R2] [R3] [R4] [CTA]. Total: ${wordCountLabel}. Tono conversacional como hablarle a un amigo. Termina con: ¿Tienes preguntas? Escríbeme y te ayudo 👇",
  ...
  try {
    ...
    result = match ? JSON.parse(match[0]) : { mapaJson: {}, guion: raw, titulo: tema, hashtags: [] }
  } catch {
    return NextResponse.json({ error: 'Error al parsear respuesta', raw }, { status: 500 })
  }
