  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto space-y-4">
        <div className="h-8 w-64 bg-[var(--bg-elevated)] animate-pulse rounded" />
        <div className="card h-48 bg-[var(--bg-elevated)] animate-pulse" />
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-start gap-3">
        <Link href="/corporate-tasks" className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors mt-1">
      ...
      <div className="card space-y-5">
        <div>
          ...
          <p className="text-[var(--text-primary)] text-sm leading-relaxed whitespace-pre-wrap">{task.description || '—'}</p>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="label mb-1">Empresa</p>
            <p className="text-[var(--text-primary)] text-sm">{task.company.emoji} {task.company.name}</p>
          </div>
          <div>
            <p className="label mb-1">Duración</p>
            <p className="text-[var(--text-primary)] text-sm">{duration} día{duration !== 1 ? 's' : ''}</p>
          </div>
        </div>

        {/* Fechas destacadas */}
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[var(--bg-base)] border border-[var(--bg-border)] rounded-lg px-3 py-2.5">
            <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider mb-0.5">Desde</p>
            <p className="text-sm text-[var(--text-primary)] font-medium">{fmtDate(task.startDate)}</p>
          </div>
          <div className="bg-[var(--bg-base)] border border-[var(--bg-border)] rounded-lg px-3 py-2.5">
            <p className="text-[10px] text-[var(--text-muted)] uppercase tracking-wider mb-0.5">Hasta</p>
            <p className="text-sm text-[var(--text-primary)] font-medium">{fmtDate(task.dueDate)}</p>
          </div>
        </div>
        ...
        <div>
          ...
          {task.employeeEmails.length === 0 ? (
            <p className="text-[var(--text-muted)] text-sm">Sin destinatarios</p>
          ) : (
            <div className="flex flex-wrap gap-1.5">
              {task.employeeEmails.map((email) => (
                <span key={email} className="text-xs px-2 py-0.5 rounded bg-[var(--bg-elevated)] border border-[var(--bg-border)] text-[var(--text-secondary)] font-mono">{email}</span>
              ))}
            </div>
          )}
        </div>
        ...
        {task.status === 'pending' && task.internalNotes && (
          <div>
            ...
            <p className="text-[var(--text-secondary)] text-sm whitespace-pre-wrap bg-[var(--bg-base)] rounded-lg p-3 border border-[var(--bg-border)]">{task.internalNotes}</p>
          </div>
        )}
      ...
      {task.instances.length > 0 && (
        <div className="card">
          <h2 className="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-wider mb-3">Instancias diarias ({task.instances.length})</h2>
          <div className="space-y-1 max-h-64 overflow-y-auto">
            {task.instances.map((inst) => (
              <div key={inst.id} className="flex items-center justify-between py-2 border-b border-[#111] last:border-0">
                <p className="text-sm text-[var(--text-primary)]">{new Date(inst.scheduledDate).toLocaleDateString('es-CO', { dateStyle: 'medium' })}</p>
        ...
        {showEdit && (
          <OverlayModal onClose={() => setShowEdit(false)}>
            <div className="p-6 max-h-[80vh] overflow-y-auto">
              <h2 className="text-lg font-bold text-[var(--text-primary)] mb-4">✏️ Editar tarea</h2>
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm text-[var(--text-primary)]">Tarea recurrente</span>
                </label>
        ...
        {showReschedule && (
          <OverlayModal onClose={() => setShowReschedule(false)}>
            <div className="p-6">
              ...
              <p className="text-[var(--text-secondary)] text-sm mb-4">{task.title}</p>
              <div className="mb-4">
        ...
        {showDelete && (
          <OverlayModal onClose={() => setShowDelete(false)}>
            <div className="p-6">
              ...
              <p className="text-[var(--text-secondary)] text-sm mb-1">¿Eliminar <strong className="text-[var(--text-primary)]">"{task.title}"</strong>?</p>
              <p className="text-[var(--text-muted)] text-xs mb-6">Esta acción no se puede deshacer. Se eliminarán todas las instancias asociadas.</p>
              <div className="flex gap-2">
